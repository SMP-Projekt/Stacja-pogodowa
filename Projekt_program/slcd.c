 ...
 *----------------------------------------------------------------------------*/
void slcdDisplay(int16_t value ,uint16_t format){	
	/* TASK 2.7: Write value to the table in specified format */
	int i = 0, k = 0, j = 0, start = 1, wynik = value;
	int16_t tab[4] = {0, 0, 0, 0};
	...
	for(i=0; i < 4; i++) slcdSet(tab[i], i+1);
}
/*----------------------------------------------------------------------------
  Clear sLCD
 *----------------------------------------------------------------------------*/
...
{
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] |= (LCD_S_DP);
}
