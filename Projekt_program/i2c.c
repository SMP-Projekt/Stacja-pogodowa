...
void initI2C(void)
{
	SIM->SCGC5 |= SIM_SCGC5_PORTE_MASK | SIM_SCGC5_PORTA_MASK;	/* Enable clock for PORTE */
...
/*----------------------------------------------------------------------------
	Write one byte of data to device register 
*----------------------------------------------------------------------------*/
...
void I2C_writeRegister_TSL(uint8_t u8SlaveAddress, uint8_t u8RegisterAddress, uint8_t u8Data)
{
	...
	I2C_writeByte(0x80 | u8RegisterAddress);						/* Send register address */
	I2C_wait();
